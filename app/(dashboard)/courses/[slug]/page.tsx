...
const SALES_CONTENT: Record<
  string,
  { exam: { about: string; facts: { label: string; value: string }[] }; whatYouGet: string[] }
> = {
  cams: {
    exam: {
      about:
      ...
      facts: [
        ...
        { label: 'This course', value: '11 modules · 49 lessons' },
      ],
    },
    whatYouGet: [
...
export default async function CourseDetailPage({ params }: Props) {
  ...
  // Enrolled student → the full course view. (Confirmed only when signed in.)
  if (user) {
  ...
  const content = SALES_CONTENT[course.slug] ?? {
    exam: { about: course.description ?? '', facts: [] },
    whatYouGet: [],
  }
