...
4. **Citation discipline — MECHANICS ARE CHECKED BY A DETERMINISTIC GATE; DO NOT RE-LITIGATE THEM HERE.** A separate gate (\`citation_bind\`), calibrated against the approved baseline lessons, validates citation granularity and the lesson-pool locatability of structured references. Do NOT issue DISAGREE for "citations are document/page-level rather than name+section", "a slide reference isn't in the citation pool", or "this claim lacks a pinpoint citation" — those are the gate's job, and the approved baseline lessons themselves flag at exactly that level (one baseline AGREE lesson had 39 of 72 references unbound by that gate). Your only citation responsibilities in THIS methodology audit: (a) confirm every reading scene carries a \`citations[]\` array at all (structural), and (b) confirm the lesson is not built on prohibited/commercial sources (per item 2). Whether a cited statute/section actually says what the lesson claims — citation FACTUAL ACCURACY — is audited in the separate factual-fidelity pass, not here.
