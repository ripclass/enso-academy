...
- **Interactive + PBL — BUILT (2026-06-21, live):** the course had **zero** interactive/pbl scenes (all reading/slide/quiz), so renderers + **content seeding** are both needed.
  - **Interactive** (`asInteractiveSpec`, `spec.kind`): `risk-classify` widget (`components/lesson/scenes/interactives/risk-classify.tsx`) — drag/tap customers into risk tiers + feedback. Live scene seeded in `customer-risk-rating-models` (order 9.5).
  - **PBL** (`asPblSpec`, `spec.kind='project'`): `project-scene.tsx` — brief + rubric + submission box; `gradeProjectSubmission` (Sonnet) grades it (band + feedback, resubmittable), feeds the knowledge model. Live "draft the SAR narrative" project seeded in `drafting-the-suspicious-transaction-report` (order 10.5).
  - To add more: build a widget / new `kind`, seed a `scene_type='interactive'|'pbl'` row (model the INSERT on an existing row; `element_type='explanation'`; `scene_type` enum already includes both; place with a fractional `metadata.order`).
- **Remaining:** more interactive kinds (transaction-network map, screening match) + more PBL scenes; **Higgsfield avatars** (Ripon: DiceBear ones are "bad" → single swap in `components/lesson/classroom/avatar.tsx`); retake-integrity fix (mock options not shuffled per attempt).
