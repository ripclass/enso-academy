...
- **Remaining:** more interactive kinds (transaction-network map, screening match) if wanted; **Higgsfield avatars** (Ripon: DiceBear ones are "bad" → single swap in `components/lesson/classroom/avatar.tsx`); retake-integrity fix (mock options not shuffled per attempt).
