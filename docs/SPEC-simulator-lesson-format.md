...
4. **The decision standard** (slide, callout template). One compact, reusable frame stated once, right after the cold open (e.g. "Predicate. Proceeds. An act on the proceeds. A culpable mental state."), which every later decision explicitly reapplies. Lowers cognitive load without reverting to lecture (external-review fix; live in all four pilots).
5. **Original teaching scenes** (readings/slides, kept verbatim). The original opener gets a 1-sentence connective prepend referencing the cold open.
6. **Mid-lesson decision(s)** (quiz, 1 question each). Either new, or a *self-contained* question redeployed from the original final check. Placed after the teaching it applies.
7. **The case file** (interactive, kind `case-file`). The lesson's deep case as 3 evidence steps: card, committed decision, reveal. See section 4.
8. **"The file, on the record"** (reading, short). Carries the deep case's citations so source discipline survives the interactive format.
9. **Classmate rep** (quiz, 1 question). A named classmate (Priya, Marcus, Aisha, Daniel, Lena, Omar) voices a plausible misconception; the student corrects them before they embarrass themselves. Usually a reframed final-check question; carry any shared scenario into the intro.
10. **Trimmed final check** (quiz). The remaining original questions. Keep multi-question applied scenarios INTACT (do not split questions that share an intro).
11. **(R) "Apply it" challenge** (existing system, only for lessons in `CHALLENGE_LESSON_SLUGS`). Injects before synthesis; coexists fine.
12. **Synthesis** (slide, kept verbatim).

Decision density target: 6-9 committed decisions per lesson, distributed, instead of one end-loaded quiz. The Apply-it challenge COUNTS toward the budget and serves as the exam-like closer of the difficulty arc; trim the final check accordingly. Every committed decision carries the confidence step (two-step commit: pick, state how sure, then the reveal) automatically at runtime.
...
- **Confidence is built in:** every case-file decision runs the two-step commit (pick, confidence chips, reveal) and reports to the player's calibration tally. Nothing to author.

**Case rotation (retakes get a different verified matter):**
- The spec takes optional `alternates: CaseFileCase[]` (each a full `{caseTitle, intro?, steps, debrief}`); one of [primary, ...alternates] is selected per lesson visit, seeded by the session id (SSR-stable; a new session per visit rotates the draw).
- The SCENE title must be neutral ("Work the file") with a rotation-aware summary; the case identity lives in the widget's own `caseTitle` header. A case-specific scene title contradicts the sidebar when the alternate is drawn.
- The "on the record" reading carries ALL cases' citations and names which set belongs to which file.
- Alternates are verified content only, never generated, and never block a conversion; the gate validates them with the same per-step rules.
- With 2 cases a repeat draw is a coin flip; a third case drops repeats to 1-in-3. Grow per the case-bank program.
...
| Confidence calibration | Two-step commit on every quiz + case-file decision; session tally (certain-but-wrong / unsure-but-right); office-hours calibration line + personal chip after 2+ overconfident misses; every stated confidence persists to session_events WITH course_id | `confidence-chips.tsx`, `quiz-scene.tsx`, `case-file.tsx`, `recordDecisionConfidence` in `lib/lesson/actions.ts`, `calibrationRef` in player, `wrap-up-panel.tsx` |
| Readiness calibration cap | Over 25% certain-but-wrong (n>=8) caps the signoff at approaching; readiness reads FULL SIMULATIONS only (practice + ladder excluded) | `updateReadiness` in `lib/mock/actions.ts` |
| Case rotation | Visit-seeded draw across [primary, ...alternates] | `caseVariantIndex` in `lib/lesson/scenes.ts`, `caseSeed` thread |
| Mock autopsy | Misses classified before the next simulation unlocks; next-3-repairs output; attempts stamped `metadata.kind` | `autopsy-panel.tsx`, `submitMockAutopsy` + gate in `lib/mock/actions.ts` |
| Error ledger | Repeat offenders (2+ misses) on the course page; Desk Mix biases by miss count first | course page, `lib/practice/desk-mix.ts` |
| Desk Mix | 8 mixed-domain committed decisions, tag revealed after commit | `/courses/[slug]/desk-mix`, `lib/practice/desk-mix.ts` |
| Flight plan / exam week | Exam date on enrollment metadata; three regimes incl. the exam-week stop rule | `flight-plan-card.tsx`, `lib/practice/flight-plan.ts` |
...
2. Write a builder script `scripts/_build-pilot-<abbrev>.js` following the existing four (`_build-pilot-wca.js`, `_build-pilot-wml.js`, `_build-pilot-cbr.js`, `_build-pilot-lesson-v2.js`). It must: read from `<artifact>.json.pre-pilot.bak` (create on first run; idempotent), reuse retained scenes byTitle, insert the decision-standard scene right after the cold open, give the case-file scene the NEUTRAL title "Work the file" with a rotation-aware summary, assert no em-dashes, assert citations on readings, mirror `conceptTags` from `teachesConcepts` on new scenes.
